...
import { getRitualById } from '../../services/ritual/ritualService.js';
...
export async function deliverRitual(bot, user, ritualId = null) {
  ...
  if (!ritualId) {
    try {
      ...
      await bot.sendMessage(chatId, `🕯️ *Your Shadow speaks:*\n\n${ritualText}`, {
        parse_mode: 'Markdown',
        reply_markup: {
      ...
      await bot.sendMessage(chatId, `⚠️ Shadow is silent. Something interfered with the ritual.`);
      return;
    }
  }
  ...
  try {
    ...
    const [shadowSnap, trophiesSnap] = await Promise.all([
      db.collection('users').doc(user.id).collection('shadow').doc('state').get(),
    ...
    if (!hasTrophies || !hasStreak) {
      await bot.sendMessage(chatId, `⛔ You are not yet worthy of this ritual.\n\nReturn when your obedience is proven.`);
      return;
    }
    ...
    await bot.sendMessage(chatId, `❌ The ritual could not be delivered. Your Shadow retreats...`);
  }
}
